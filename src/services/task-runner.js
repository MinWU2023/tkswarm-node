...
function event(taskId, message, level = 'info') {
  db.prepare('INSERT INTO task_events(task_id,level,message) VALUES (?,?,?)').run(taskId, level, message);
}
...
async function execute(task) {
  ...
  event(task.id, `任务开始执行，共 ${accounts.length} 个账号`);
